...
| `KD_GAT_RECIPE`        | `configs/recipes/ablation.jsonnet` | `component.py:RECIPE_PATH` |

Dev-path config overrides: pass `--tla key=value` flags (JSON-encoded) for
jsonnet top-level args, or `--set dotted.key=value` to patch the rendered
dict after validation. The pre-Phase-3 `env_prefix="KD_GAT"` jsonargparse
auto-ingest (`KD_GAT_MODEL__INIT_ARGS__LR=0.001`) was deleted along with
`LightningCLI` — use `--set model.init_args.lr=0.001` instead.
...
### HPC resource profiles (`configs/resources/job_profiles.json`)
...
Source of truth lives in `configs/resources/job_profiles.json` (per
family/scale/stage, and per-method for fusion). The resolver uses this file
for SLURM sizing.
...
| Jsonnet                  | model_type | embeddings | attention | cka | landscape  | fusion_policy |
|--------------------------|-----------|------------|-----------|-----|------------|---------------|
| `analyze_vgae.jsonnet`   | vgae      | yes        | —         | —   | yes (51×51)| —             |
| `analyze_gat.jsonnet`    | gat       | yes        | yes       | yes | yes        | —             |
| `analyze_fusion.jsonnet` | fusion    | —          | —         | —   | —          | yes           |
...
submit.sh <profile> [args...]
  → reads submit profile from configs/resources/submit_profiles.json
...
| Temporal model has no stage jsonnet | `config/stages/` | Can't launch via `--config` pattern |
| `dgi/large` declared in `pipeline.yaml` but no model config or resource profile | `pipeline.yaml` + `models/dgi/` + `configs/resources/job_profiles.json` | Runtime `KeyError` |
| `bandit` resource profiles never reached | `configs/resources/job_profiles.json` | `get_resources` receives `model_type="dqn"` for all fusion |
| `medium` scale entries in `configs/resources/job_profiles.json` have no `pipeline.yaml` counterpart | `configs/resources/job_profiles.json` | Dead config |
| `batch_size=8192` is dead config | `datamodule.py` | Bypassed by `DynamicBatchSampler` when `dynamic_batching=True` |
| `num_workers` mismatch | `configs/resources/job_profiles.json` vs stage jsonnet | 1 CPU wasted per job |
...
| No `analyze_dgi.jsonnet` | `config/stages/` | DGI analyzer would work with `model_type=vgae` but no config |
| No `analyze_temporal.jsonnet` | `config/stages/` | No analyzer support for temporal model |
