...
|---|---|---|
| Render plan | `graphids run <plan> -o plan.json` | `<plan>` is a dotted module under `graphids.plan.plans` (e.g. `ofat`, `smoke.gat_taunorm`). Imports + calls `build(dataset, seed)`, validates as `Plan`, writes JSON. No submit, no MLflow query. |
...
- **Per-pipeline submission flags.** No `--ofat-mode`, no `--sweep-strategy`.
  ...
  New declarative state goes on `TrainRow` / `Plan`
