...
def _teacher(scale: str = "large", stages=None) -> dict:
    return {
        "stages": list(stages or ["supervised"]),
        "scale": scale,
        "model_type": "gat",
    }
...
class TestKDTeacherResolution:
    def test_explicit_teacher_config_wires_upstream(self):
        ...
        teacher_assets = [
            a for a in student.upstream_asset_names if a.startswith("supervised") and "_kd" not in a
        ]
        assert teacher_assets, f"teacher upstream not wired; got {student.upstream_asset_names}"
        ...
        recipe = _recipe(
            {
                ...
                "bad_teacher_has_aux": _student(
                    teacher_config="grandteacher",
                    scale="large",
                ),
                "student": _student(
                    teacher_config="bad_teacher_has_aux",
                    scale="small",
                ),
            }
        )
        ...
        recipe_a = _recipe(
            {
                ...
                "beta_large": _teacher(scale="large", stages=["supervised"]),
