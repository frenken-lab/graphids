"""Materialization contract tests for supported graph representations."""
...
def _build_tables(df: pl.DataFrame, representation_cfg):
...
def test_sequence_labels_use_target_window_not_any_context_window():
    ...
    assert _build_tables(context_only_attack, cfg).labels["y"].to_list() == [0, 0]
    ...
    assert _build_tables(target_attack, cfg).labels["y"].to_list() == [1, 0]
