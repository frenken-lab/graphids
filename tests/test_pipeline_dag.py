...
def test_variants_reference_valid_stages():
    import yaml
    from graphids.config.constants import CONFIG_DIR, STAGES
    pipeline = yaml.safe_load((CONFIG_DIR / "pipeline.yaml").read_text())
    for name, v in pipeline["variants"].items():
        bad = [s for s in v["stages"] if s not in STAGES]
        assert not bad, f"Variant '{name}' has unknown stages: {bad}"
