...
@app.command("mlflow-start-parent", rich_help_panel="Meta")
def mlflow_start_parent(
    group: Annotated[str, typer.Option("--group", help="Ablation group (e.g. conv_type)")],
    variant: Annotated[
        str, typer.Option("--variant", help="Variant within the group (e.g. gatv2)")
    ],
    dataset: Annotated[str, typer.Option("--dataset", help="Dataset name (e.g. set_01)")],
    cluster: Annotated[str, typer.Option("--cluster", help="Target cluster name")] = "",
    seeds: Annotated[
        str, typer.Option("--seeds", help="Comma-separated seeds; stamped as a tag")
    ] = "",
) -> None:
    """Open + close an MLflow parent run; print its run_id to stdout.

    Launchers capture the printed run_id and export it as
    ``MLFLOW_PARENT_RUN_ID`` for child SLURM jobs, which stamp it as
    ``mlflow.parentRunId`` at ``start_run`` time. Exits 1 (empty stdout)
    on MLflow failure so the launcher can fall back to un-grouped children.
    """
    from graphids._mlflow import start_parent_run

    child_seeds = [int(s) for s in seeds.split(",") if s.strip()] if seeds else None
