...
from graphids.core.data.preprocessing.segments import EntitySegmentCfg, MultiScaleSegmentCfg, SequenceSegmentCfg, WindowSegmentCfg
...
def build_tables(pipeline: GraphPipeline, df: pl.DataFrame, window_size: int, stride: int):
    return build_graph_tables(
        ...
        segment_cfg=pipeline.segment_cfg,
    )


def inspect(pipeline: GraphPipeline, df: pl.DataFrame, window_size: int, stride: int) -> dict[str, pl.DataFrame]:
    tables = build_tables(pipeline, df, window_size, stride)
    return {
...
def run(pipeline: GraphPipeline, df: pl.DataFrame, window_size: int, stride: int) -> tuple[Data, dict, int, int]:
    tables = build_tables(pipeline, df, window_size, stride)
