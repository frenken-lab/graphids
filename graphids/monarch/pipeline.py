...
from graphids.monarch.schemas import PipelineConfig, SweepConfig
...
def run_chain(
    ...
    max_retries: int = 2,
    lake_root: str = "",
    job_spec_override: Any | None = None,
) -> dict[str, str]:
    ...
    if job_spec_override is not None:
        spec = job_spec_override
    else:
        spec = chain_job_spec(
...
def run_pipeline(
    ...
    *,
    ...
    Entry point for ``monarch-run``. Builds StageConfigs via the planner
    and delegates to ``run_chain``. Pass ``job_spec_override`` to use a
    custom ``MonarchJobSpec`` (e.g. --partition/--time CLI overrides).
    """
...
def _run_with_retry(fn: Any, stage_name: str, max_retries: int) -> str:
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            last_tb = traceback.format_exception(exc)
            log.error(
                "stage_failed",
                stage=stage_name,
                attempt=attempt,
                error=str(exc),
                traceback="".join(last_tb),
            )
