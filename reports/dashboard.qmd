...
filteredChart("training_curves", sel => vg.plot(
  vg.cell(
...
filteredChart("attention_weights", sel => vg.plot(
  vg.cell(
...
filteredChart("dqn_policy", sel => vg.plot(
  vg.rectY(
...
filteredChart("recon_errors", sel => vg.plot(
  vg.rectY(
...
filteredChart("cka_similarity", sel => vg.plot(
  vg.cell(
...
filteredChart("embeddings", sel => vg.plot(
  vg.dot(
...

{
  const sel = vg.Selection.intersect(
    vg.Selection.single({ init: { model_type: landscapeModel } }),
    vg.Selection.single({ init: { dataset: landscapeDataset } })
  );

  return vg.plot(
    vg.raster(
      vg.from("loss_landscape", { filterBy: sel }),
      {
        x: "x",
        y: "y",
        fill: vg.avg("loss"),
    ...
    vg.colorLegend(true),
    vg.colorLabel("Loss"),
    vg.xLabel("Direction 1"),
    vg.yLabel("Direction 2")
  );
}
```
