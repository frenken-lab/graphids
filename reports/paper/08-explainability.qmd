...
renderSpec(await FileAttachment("figures/fig-umap-vgae.json").url())
```
...
renderSpec(await FileAttachment("figures/fig-umap-gat.json").url())
```
...
renderSpec(await FileAttachment("figures/fig-recon-errors.json").url())
```
...
renderSpec(await FileAttachment("figures/fig-graph-stats-boxplot.json").url())
```
...
import { renderAdjacencyMatrices as renderAdjMat } from "../_ojs/graph-analysis.js"
adj_graph_samples = FileAttachment("../data/graph_samples.json").json()

{
  return renderAdjMat(Plot, adj_graph_samples.data, { width: 700, maxGraphs: 4 });
}
```
...
renderSpec(await FileAttachment("figures/fig-attention-heatmap.json").url())
```
...
renderSpec(await FileAttachment("figures/fig-dqn-policy.json").url())
```
