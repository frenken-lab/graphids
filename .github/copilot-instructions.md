...
- **Multi-stage chains:** Declarative DAG in `graphids.slurm.dag.OFAT_DAG` (CLI: `python -m graphids launch-ablation`). Executor holds jids in-memory and submits each node via `scripts/run` with `SBATCH_DEP=afterok:<jid>`. MLflow's per-variant `status=FINISHED` drives idempotent skip on re-launch.
